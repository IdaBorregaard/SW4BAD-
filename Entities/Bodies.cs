...
public class CelestialBody
{
    ...
    [Range(0, float.MaxValue)] // Validation to ensure distance is non-negative
    ...
    // Relations
    public List<Mission> TargetMissions {get; set;} = null!; // Missions that have this body as their target
    public string? ParentPlanetName {get; set;} // Foreign key to reference the parent planet (if this body is a moon)

    [ForeignKey("ParentPlanetName")]
    public CelestialBody? ParentPlanet {get; set;} // Navigation property to the parent planet (if this body is a moon)
    public List<CelestialBody> Moons {get; set;} = new(); // List of moons orbiting this body (if this body is a planet)
}